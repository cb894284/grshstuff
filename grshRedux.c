...
int process(char *buffer) {
	...
	if (strstr(buffer, ">") != NULL) { //REDIRECT
		...
		int x = 0;
		char* tokenize[sizeof(arguments[1])];
		tokenize[0] = strtok(arguments[1], " \n\t");
	...
	if (buffer[0] != '\0' && buffer[0] != '\n') {
		...
		if (strcmp(command[0], "cd") == 0) {//cd
			...
			if (pathEmpty == 1)
				write(STDERR_FILENO, ERROR_MESSAGE, strlen(ERROR_MESSAGE));
			else //Finally time to fork and exec!
				rc = fork();
				if (rc < 0) { //Fork Error
					printError();
					exit(1);
					...
					if (pathChanged == 0) {
						path = strdup("/bin/"); //by default, the path starts at /bin/
					...
					if (execv(path, command) == -1) {//successfuly executed binary or not? 
						printError();
						exit(0);
					}
				}
